...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">Reports</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ Clients</span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            @can('Export Invoices Excel')
                @if(Route::is('reports.clients.index'))
                ...
                @else
                    <div class="pr-1 mb-3 mb-xl-0">
                        <form action="{{route('reports.clients.export')}}" method="post">
                            ...
                            <button type="submit" class="btn btn-success-gradient">Export Excel</button>
                        </form>
                    </div>
                @endif
            @endcan
            <div class="pr-1 mb-3 mb-xl-0">
                <button class="btn btn-info-gradient" onclick="printInvoicesSearchResult()">Print</button>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                <form action="{{route('reports.clients.search')}}" method="post">
                    @csrf
                    <div class="row row-sm px-4 pt-4">
                        <div class="col-lg">
                            <label>Select Department<span class="tx-danger">*</span></label>
                            <select id="department_search_value" onchange="getProductsOfDepartment(this.value)" name="department_id" class="form-control select2-no-search">
                        ...
                        <div class="col-lg mg-t-10 mg-lg-t-0">
                            <label>Select Product<span class="tx-danger">*</span></label>
                            <select id="product_search_value" name="product_id" class="form-control select2-no-search">
                            </select>
                        </div>
                        <div class="col-lg mg-t-10 mg-lg-t-0">
                            <label>From Date</label>
                            <input class="form-control fc-datepicker" name="date_from" value="{{$date_from ?? ''}}" placeholder="YYYY-MM-DD" type="text">
                        </div>
                        <div class="col-lg mg-t-10 mg-lg-t-0">
                            <label>To Date</label>
                            <input class="form-control fc-datepicker" name="date_to" value="{{$date_to ?? ''}}" placeholder="YYYY-MM-DD" type="text">
                        </div>
                    </div>
                    <div class="pl-4 pt-2">
                    ...
                    <div class="table-responsive">
                        <table class="table text-md-nowrap" id="example1">
                            ...
                            <tr>
                                ...
                                <th id="actions_head_column" class="wd-20p-f wd-lg-10p-f border-bottom-0">Actions</th>
                            </tr>
                            </thead>
                            <tbody>
                            @php $i=0; @endphp
        ...
        function getProductsOfDepartment(departmentId){
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    document.querySelector('#product_search_value').innerHTML = products;
                })
        }

        function printInvoicesSearchResult(){
            document.body.innerHTML = document.querySelector('#example1_wrapper').children[1].innerHTML;
            window.print();
            location.reload();
        }
    </script>
